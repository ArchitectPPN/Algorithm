 ...
 * 3. 由于需要反转链表， 这里将当前节点与它的下一个节点断开连接， 这里使用head->next = $tail， 第一个元素时等价于head->next = null，
 * 然后我们更新tail，tail=head；这样tail就保存了反转后的链表； 用例子来解释： 原先的链表是1->2->3->4->null 走到第一步时， tail = 1 -> null,
 * 原先的链表为：2->3->4->null; 接着循环2步时， tail = 2 -> 1 -> null, 原先的链表为：3->4->null; 循环3步时， tail = 3 -> 2 -> 1 -> null,
 * 原先的链表为：4->null; 循环4步时， tail = 4 -> 3 -> 2 -> 1 -> null, 原先的链表为：null; 这个解法看起来就是，在循环过程中，一个一个处理当前节点：
...
$svc = new ReverserLinkedList();
$svc->reverse((new BuildLinkedList())->buildLinkedList([1,2,3]));