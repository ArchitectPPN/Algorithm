...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "get_file_info",
            "description": "获取指定文件的基本信息（大小、修改时间等）。",
            "parameters": {
                "type": "object",
                "properties": {
                    "file_path": {
                        "type": "string",
                        "description": "文件路径，如 'react_minimal_demo.py'"
                    }
                },
                "required": ["file_path"]
            }
...
def do_get_file_info(file_path: str) -> str:
    """获取文件信息"""
    ...
    return f"文件：{file_path}，大小：{size_kb:.1f}KB，最后修改：{mtime}"
...
messages = [
    {"role": "user", "content": "现在几点了？另外帮我算一下 15 的平方根加上 8 等于多少，还有 react_minimal_demo.py 这个文件有多大？"}
]
...
while loop_count < MAX_LOOPS:
    ...
    for i, m in enumerate(messages):
        ...
        if m.get("tool_calls"):
            ...
            print(f"    [{i}] role={role}, tool_calls={tc_names}")
        elif content:
            preview = content[:80] + ("..." if len(content) > 80 else "")
            print(f"    [{i}] role={role}, content={preview}")
        else:
            print(f"    [{i}] role={role}, (无内容)")

    print(f"\n  📥 模型响应:")
    if msg.get("tool_calls"):
        for tc in msg["tool_calls"]:
            print(f"    tool_call: name={tc['function']['name']}, arguments={tc['function']['arguments']}, id={tc['id']}")
        if msg.get("content"):
            print(f"    content: {msg['content'][:100]}...")
    else:
        print(f"    content: {msg.get('content', '(无文字)')}")
