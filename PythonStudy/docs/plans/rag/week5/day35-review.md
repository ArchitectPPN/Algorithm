...
把文本变成一串**数字向量**（768 维），**语义相近的文本 → 向量距离近**。字数和维度无关——9 字的查询和 500 字的片段都是 768 维，在同一向量空间里才能比相似度。它是压缩不是拆分：一段话压成一个坐标点，语义近就靠得近。

**Q2. 为什么需要分片？**

太长 → **语义稀释**：一条向量装不下整本书的信息，查询只命中片段一小部分，相似度被拉低；太短 → **上下文丢失**：片段信息不完整，检索到了也答不出。所以 `chunk_size=500`（+overlap=50）是在"语义完整"和"检索精度"间取平衡。

**Q3. Chroma 和传统数据库的区别？**

传统数据库存**结构化数据**，用精确条件查询（`WHERE x = y`）；Chroma 存**向量**，用相似度查询（ANN 近似最近邻），返回"最像的" Top-K，不做精确匹配。Chroma 一条记录存 4 样：ids / embeddings / documents / metadatas——**向量负责"找"，原文负责"用"**。

**Q4. RAG 的完整流程？**

"加载 → 分片 → 向量化 → 存库（向量+原文+元数据）→ 查询转向量 → 余弦粗筛（阈值 0.5）→ LLM 精筛（判相关）→ 相关片段为空则拒绝回答 → 否则喂给 LLM 基于片段生成回答"（详见 `day34-pipeline.md`「全流程逻辑（严谨版）」）。
