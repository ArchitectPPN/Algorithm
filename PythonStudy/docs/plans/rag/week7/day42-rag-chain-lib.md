...
| 查询 | 命中来源 | 距离 | empty | 回答质量 |
|------|---------|------|-------|---------|
| 如何防止 SQL 注入 | sql-best-practices #0/#1 + cache-design #0 | 0.31/0.38/0.41 | False | ✅ 正确提到参数化查询，给了代码示例 |
| Git 分支应该怎么管理 | git-workflow #0/#1 + log-best-practices #1 | 0.30/0.45/0.51 | False | ✅ 结构完整（分支策略/commit/PR/审查），紧扣知识库 |
| 今天下午吃什么 | error-handling #1 + cache-design #0 | 0.59/0.59 | **False** | ⚠️ LLM 正确拒答，但 empty 标记不对 |

### ⚠️ 发现的问题：阈值 0.6 对完全无关查询偏松

"今天下午吃什么"检索到 2 条噪音（距离 0.588/0.591，刚好低于 0.6），导致 `empty=False`。
LLM 虽然凭 prompt 约束正确拒答了，但 **empty 标记和 sources 不可信**——下游若用 empty 判断"有没有相关信息"会被误导。

根因：bge-m3 对中文短句的余弦距离分布偏紧（无关内容也在 0.5-0.6 区间），0.6 这个阈值是 Day40 按"弱相关"定的，对"完全无关"不够严。

修法（Day44 接入 Agent 时再定）：
- 方案 A：阈值降到 0.5（但可能误杀弱相关规范）
- 方案 B：保留 0.6，下游不依赖 empty，靠 LLM 判断拒答（当前已能做到）
- 方案 C：加一层"查询-片段 语义相关度"二次判断（贵）

### 阈值实验（max_distance 对 empty 和噪音的影响）

| max_distance | "今天下午吃什么" 检索条数 | empty | 噪音 |
|-------------|----------------------|-------|------|
| 0.3（严） | 0 条 | ✅ True | 无 |
| 0.6（默认） | 2 条 | ❌ False | 2 条噪音 |
| 0.9（松） | 3 条 | ❌ False | 3 条噪音 |

> 结论：阈值越严越能挡住无关查询，但也可能误杀弱相关规范。
> 0.3 对"完全无关"有效但太严（正经查询的弱相关片段也会被砍）；
> 0.9 基本不过滤。0.6 是"保留弱相关、放过明显无关"的折中，但对 bge-m3 的中文短句仍偏松。
> Day44 接入 Agent 后会重新评估——Agent 场景下 LLM 自己会判断片段相不相关，对阈值的依赖比固定管道弱。

### 3b vs 8b 模型对比（同一查询"如何防止 SQL 注入"）

| 维度 | qwen2.5:3b | qwen3:8b |
|------|-----------|---------|
| 耗时 | 23.5s | **105.8s**（4.5 倍） |
| 回答结构 | 平铺直叙，一段话 | 分点编号，结构化 |
| 代码示例 | ✅ 给了 `cursor.execute(... %s, (id,))` | ✅ 给了同样示例，还标了 ❌/✅ |
| 覆盖面 | 参数化查询一点 | 参数化 + ORM + 输入校验三层 |
| 边界意识 | 无 | ✅ 主动注明"知识库未提及其他手段" |

> 结论：8b 回答更结构化、覆盖更全、有边界意识（主动声明知识库局限），但**慢 4.5 倍**。
> Day44 审查 Agent 选 8b：代码审查需要更强推理和多角度覆盖，慢一点可接受；
> Day40 的 /ask 问答用 3b：问答场景快比全更重要。**模型选型看场景，不是越贵越好。**

### 温度稳定性实验（2026-09-02 补充，学习者亲手采样）

**起因**：执行 `rag_chain.py` 时出现"自相矛盾"输出——正文正确答了参数化查询（内容全来自检索片段），
末尾却跟了一句"知识库中没有相关信息"。

**根因两层**：
1. LLM 逐 token 生成、**无全局一致性检查**——每步只保证"局部像话"，不保证前后不打架；
   `/api/generate` 没有 stop 锚点，答完不知道停，prompt 里那句拒答语规则是上下文中现成的续写候选
2. `_call_llm` 未设 temperature（ollama 默认 0.8）——"补拒答语"是分叉点上的低概率分支，被抽中

**实验判据的演进（本实验最有价值的部分）**：
- v1 判据：数"拒答语出现次数"——低频事件，小样本功效不足（10 次全 0 也不能下结论，
  异常率 3% 时 10 次全正常概率约 74%）
- v2 判据：数"唯一回答文本数"（md5 判重，只取"回答"段落排除耗时数字污染）——
  温度的高频直接效应（措辞浮动），**每次采样都产生信息**

**数据**（qwen2.5:3b，同一查询"如何防止 SQL 注入"）：

| 组 | temperature | 采样次数 | 唯一版本数 | 拒答语异常 |
|----|------------|---------|-----------|-----------|
| v1 | 1 | 10 | — | 1 次 |
| v2 | 1 | 20 | **20**（每次都不同，字数 128-381 波动近 3 倍） | 0 次 |
| v2 | 0 | 30 | **2**（仅首跑 153 字，后续 29 次 154 字一字不差） | 0 次 |

**分层结论**：
1. **温度是输出不稳定的主要来源**：t=1 下 20/20 全不同 → t=0 后 30 次几乎全同
2. **t=0 是工程级稳定，不是数学级确定**：首跑与热跑差 1 字——推理引擎非确定性
   （批处理浮点归约顺序 → 概率微扰 → 候选打平处 argmax 翻转）。审查场景完全够用，
   绝对可复现才需要 seed/锁批处理等手段
3. **拒答语异常实测率 ~3%**（30 次里 1 次）：低频事件小样本抓不到，
   这就是判据从"数异常"升级到"数版本"的原因

**修复**：`_call_llm` 加 `"temperature": 0`（已改）。与 Day44 结论互证：
所有需要稳定输出的 LLM 调用必须 t=0，否则行为不可复现、对照实验无效。

**方法论沉淀**：
- 单次结果 ≠ 频率分布（掷一次骰子没出 6 ≠ 骰子没有 6）
- **判据选择比采样次数更重要**：v1 判据下那 20 次实验"一无所获"（异常 0 次），
  v2 判据同一批数据直接把温度效应拍在桌上（20 个版本）
- 模型越小温度放大越明显：3b 每步分布更平（信念弱），t=1 下字数波动近 3 倍——
  与"3b 指令遵循弱"同根同源

## 产出文件

- `rag/rag_chain.py`（含 temperature: 0 修复）
- `run10.py`（温度采样实验脚本 v2：数唯一版本数判据）
- 本文件"实验结果"小节（含 2026-09-02 温度实验）
