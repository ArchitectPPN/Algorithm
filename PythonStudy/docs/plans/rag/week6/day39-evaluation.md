...
运行 `python rag/evaluate.py`（基线）→ `python rag/query_rewrite.py`（同义词扩展）→ `python rag/query_rewrite.py --hyde`（HyDE）。
数据：知识库 11 文档 23 分片（Day38 扩充），15 条查询（Day38 定义），bge-m3（Day38 选型结论）。

| 方法 | Recall@5 | MRR | Top-1 |
|------|----------|-----|-------|
| 基线（原查询） | 93% (14/15) | 0.836 | 80% |
| 同义词扩展 | 93% (14/15) | 0.883 | 87% |
| **HyDE** | 93% (14/15) | **0.900** | 87% |

### 3 条问题查询的改写前后明细

Day39 评估定位到 3 条问题查询（1 条未命中 + 2 条排位靠后），对比三种方法对它们的处理：

| 问题查询（期望文件） | 基线 | 同义词扩展 | HyDE | 评价 |
|---------------------|------|-----------|------|------|
| 上线后怎么知道服务出问题（deployment-checklist） | 第3名 | **第1名** ✅ | 第2名 | 扩展最有效："上线后→部署后/生产环境/发布后"、"服务出问题→监控/告警/ERROR/故障"精准命中 |
| 登录密码怎么安全存储（sql-best-practices） | 第5名 | 第4名 | **第1名** ✅ | HyDE 最有效：假设答案写"密码哈希存储/bcrypt/argon2"≈文档原文 |
| 请求太慢怎么排查（log-best-practices） | 未命中 ❌ | 未命中 ❌ | 未命中 ❌ | 两种都救不回：测试集歧义（见发现 3） |

> 观察：**两种方法互补**——同义词扩展对"词面能对上"的查询（上线后→部署后）有效；
> HyDE 对"语义深层相关"的查询（密码安全→哈希存储）有效。最佳实践是两者结合用。

### 关键发现

**1. 排序结论**：HyDE > 同义词扩展 > 基线（MRR 0.900 > 0.883 > 0.836），验证"改查询比改模型快、零成本见效"。

**2. HyDE 救回了「登录密码怎么安全存储」**：排第5 → 排第1 ✅
   假设答案写了"密码哈希存储、bcrypt/argon2"，几乎就是 `sql-best-practices.md` 原文——
   完美印证原理：答案和文档同为完整句子，向量空间更近。

**3. 「请求太慢怎么排查」两种方法都救不回**：这是**测试集歧义**而非检索问题。
   查询指向"性能排查"，但知识库里 `log-best-practices.md`（日志）、`sql-best-practices.md`（查询优化）、`cache-design.md`（缓存）全是"性能"强干扰，期望命中的日志文档反而不是性能主题。
   这种不是改查询能解决的 → 应调整期望文件或补文档，而不是继续改查询。

**4. 方法论**：改写只能解决"用词对不上"，解决不了"语义指向别处"。
   先用失败分析区分：词对不上 → 改写；语义歧义 → 改测试集/补文档。

### 结论

bge-m3 基线已经很稳（MRR 0.836、Top-1 80%）。查询改写（HyDE）零成本把 MRR 提到 0.900、Top-1 提到 87%，且**不需要换模型**——RAG 优化优先级：先改查询 → 再调参 → 最后才换模型/加精筛。

---

## 产出文件

- `rag/evaluate.py` — 检索效果评估脚本（已跑通，指标 Recall@K / MRR / Top-1）
- `rag/query_rewrite.py` — 查询改写实验（同义词扩展 + HyDE，均已跑通）
