...
## 学习理解深化（2026-08-12 补充）

> 用"点菜类比"把 MMR 从公式讲到直觉，重点解决三个理解盲区：①MMR 到底解决什么问题；②λ 是什么、怎么调；③为什么冗余度取"最大"而非"平均"。

### 先建立直觉：点菜类比

假设有一桌菜（候选片段），你偏好荤菜（查询）。只看相关性你会点三份红烧肉——都好吃但完全重复。MMR 在"相关性"之外多加了"别和已点重复"的惩罚，逼着你点出 **红烧肉 + 糖醋鱼 + 清炒时蔬**，既好吃又覆盖不同口味。
...
| --- | --- |
| 菜 | 知识库片段 |
| 好吃程度（想吃） | 片段 和 查询 的相关性 |
| "和已点菜重不重复" | 片段 和 已选片段的相似度（冗余度） |
| 第一轮先找最想吃的 | 先选和查询最相关的 Top-1 |
| 贪吃 vs 猎奇的比例 | λ（平衡参数） |

**本质**：普通 Top-K 只优化"结果和查询的相关性"，不优化"结果之间的多样性"。MMR 两者兼顾。

### λ 到底是什么（重点）

MMR 同时想要两件**冲突**的事：高相关（和查询像）+ 高多样（彼此不重复）。很难两全：SQL 注入相关片段天生堆在一起，想多相关就得接受重复，想多多样就得接受有时不那么相关。

**λ 就是"相关 vs 多样"这个天平上，相关性所占的权重（0~1）**，唯一作用就是分权重：

```text
得分 = λ × 相关性  −  (1−λ) × 冗余度
          ↑              ↑
     给"相关"的权重   给"多样"的权重       ← 两者相加恒等于 1
```

| λ | 相关乘数 | 多样乘数 | 行为 |
| --- | --- | --- | --- |
| λ = 1 | 1 | 0 | **退化成普通 Top-K**（多样权重归 0） |
| λ = 0.7~0.8 | 高 | 低 | 常用：保证相关、挤掉部分重复 |
| λ = 0 | 0 | 1 | **只拼不重复**，可能选出一堆不相关 |
| λ = 0.4 | 低 | 高 | 偏多样，宁可吃点不相关的 |

**记法**：λ 乘在"相关性"前面，所以 λ 大 → 偏相关；λ 小 → 偏多样。λ=1 等于关掉 MMR，λ=0 等于丢掉相关性。

### 冗余度为什么取"和已选集合的最大相似度"（max_dup）？

主观上容易想取"平均相似度"，但**必须取最大**：

- **取最大** = 只要我和已选中**任何一个**高度重复，就该被强惩罚，即使我跟其他人都不同。防止"和某片段完全重复"混进来。
- **取平均** = 会被"大多不相似"稀释掉单个高度重复风险。比如某候选跟已选 A 相似 0.95（几乎没差别），但跟其余 5 段都 0.1，平均 0.24，看起来"挺多样"，实际跟 A 完全重复。

**取最大才是"防重复"的正确姿势**：只要重复任意一段就该被压，不管其余。

### 为什么第一轮只看相关性？

第一轮还没有"已选集合"可比较，冗余度那一项是 0，得分 = `λ×相关性`，λ 是常数不影响排序 → 等价于只按相关性选。本质：**没有已选集合，"别重复"无从谈起。**

### 检验理解（自测）

**Q1**：第一轮为什么看相关性不需要管冗余度？
> 还没有任何已选片段，冗余度为 0，得分=λ×相关性，λ 常數不改变排序 → 等价于纯按相关性选 Top-1。

**Q2**：第二轮 A 相关 0.9 但重复 0.95，B 相关 0.8 但重复 0.2，选谁？
> B。A 虽相关略高，但重复惩罚极重；B 相关略低但几乎不重复 → MMR 牺牲一点相关性换更大多样性。

**Q3. λ=1 会发生什么？**
> 冗余度项成 (1−1)=0，退化成普通 Top-K，只按相关取前 K → MMR 完全失效，回到"全是红烧肉"。

**Q4. λ=0 呢？**
> 相关性项成 0，得分只剩 −(1×冗余度)，完全不管相关，可能堆出不相关但互相不重复的片段 → 乱选。

**Q5. 冗余度为什么取最大不取平均？**
> 取最大防"和任何一段重复"，取平均会被大多相似稀释掉单个高重复风险。取最大才能真正防重复。
