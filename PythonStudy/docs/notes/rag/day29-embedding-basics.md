# Day 29：Embedding 原理 + 手写余弦相似度

> 目标：理解 Embedding 是什么，能用手写余弦相似度比较两段文字的语义距离。
...
```
概念（10min）→ 手写余弦相似度（20min）→ ollama 验证（15min）→ 对比实验（15min）
```
...
1. **默写**：能写出 `cosine_similarity()` 函数
2. **一句话**："Embedding 把文字变成向量，语义相近的向量距离近，用余弦相似度衡量"
3. **追问**：
   - 为什么不用关键词匹配？（同义词匹配不到）
