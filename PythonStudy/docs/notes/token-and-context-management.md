...
实测（tiktoken cl100k_base + DeepSeek 1.1 系数）：

| 文本 | 字符数 | token 数（tiktoken） | DeepSeek 调整后 | 费用（DeepSeek 输入价） |
|------|--------|---------------------|----------------|---------------------|
| 5000 字中文 | 5,000 | 3,508 | 3,859 | ¥0.0039 |
| 5000 字英文 | 5,000 | 2,605 | 2,605 | ¥0.0026 |

**结论**：
- 相同字符数下，**中文 token 消耗是英文的 7.3 倍**
- 按 token 计费，**中文 API 费用是英文的 7.9 倍**

### 原因分析

**1. 分词器词表偏好**：
- 英文：常见词（"hello"、"the"、"and"）在词表里，整体一个 token
- 中文：分词器基于英文训练，中文常用词（"你好"、"的"）反而被拆碎

**2. 实测拆分示例**：

```
"hello"       → 1 token   （5字符 → 1token，ratio=0.2）
"你好"         → 2 token   （2字符 → 2token，ratio=1.0）
"人工智能"     → 5 token   （4字符 → 5token，ratio=1.25）

"hello world" → 2 token   （11字符 → 2token，ratio=0.18）
"你好世界"     → 4 token   （4字符 → 4token，ratio=1.0）
```

**3. 信息密度对比**：
- 英文 1 token ≈ 0.25 个英文单词（3-4 个字符）
- 中文 1 token ≈ 0.8-1.0 个汉字

虽然中文 token 数更多，但信息密度高——**达到相同表达效果时，中文总 token 数可能更少**。

### 实际影响

**成本**：
- 发送相同字符数：中文更贵（token 更多）
- 发送相同语义内容：差异不大（中文信息密度高）
...
1. **监控 token 占用**：中文项目要更早触发压缩阈值
2. **工具结果摘要化**：中文工具输出的 token 浪费更严重
3. **系统 prompt 精简**：中文 system prompt 消耗更高
4. **考虑模型差异**：DeepSeek 对中文更友好（1.1 系数 vs Claude 1.3）
...
10. **生产组合拳**：先截断 → 再滑动窗口 → 再摘要 → 最后兜底重试，逐级升级
11. **70%阈值设计**：不是拍脑袋——1.3系数×70%≈91%实际触发率，留9%给极端误差+输出空间
12. **语言差异**：相同字符数下，中文 token 消耗是英文的 7 倍左右，API 费用更高
...
大模型标注的上下文长度（如 128K、192K、256K）指的是 **token（词元）数量**，**不是字符数量**。

模型通过分词器将文本转换为 token 进行计算，因此：
- **上下文限制的本质是 token 数量上限**
- 系统不关心你发送了多少字符，只关心转换后的 token 数

### 不同语言的实际字符限制

| 上下文窗口 | 英文（1 token ≈ 4 字符） | 中文（1 token ≈ 1.5-2 字符） |
|-----------|------------------------|---------------------------|
| 128K token | ~50 万字符 | ~8.5 万字符 |
| 192K token | ~75 万字符 | ~12.8 万字符 |
| 256K token | ~100 万字符 | ~17 万字符 |

**关键认知**：
- 英文等语言：发送的字符数可以远超标注的上下文（如 50 万字符对应 128K token）
- 中文：因为 token/字符比高，实际能发送的字符数往往小于标注的上下文
...
# ❌ 错误：按字符估算
if len(text) > 128000:
    # 这是错的！128K 是 token，不是字符

# ✅ 正确：用 tokenizer 估算 token 数
...
11. **70%阈值设计**：不是拍脑袋——1.3系数×70%≈91%实际触发率，留9%给极端误差+输出空间
12. **语言差异**：相同字符数下，中文 token 消耗是英文的 7 倍左右，API 费用更高
13. **上下文窗口本质**：指的是 token 数，不是字符数。中文能发送的字符数通常小于标注的上下文窗口
